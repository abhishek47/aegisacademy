        ...
        <ul class="mt-2 list-reset">
            ...
            @foreach($chapter->sections as $section)
                <li class="chapter-item mt-3 text-md">
                    <a href="/courses/{{$course->slug}}/chapter:{{$chapter->slug}}/section:{{$section->slug}}" class="no-underline font-semibold tracking-wide hover:text-grey-darker leading-normal {{ request()->is('*/section:' . $section->slug) ? 'text-grey-darker' : 'text-grey' }}">
